...
const StaffView: React.FC<StaffViewProps> = ({ data, onViewProfile }) => {
  ...
  const handleExpertiseChange = (courseId: string) => {
    const newExpertise = formState.expertise?.includes(courseId)
      ? formState.expertise.filter((id: any) => id !== courseId)
        ...
        {staff.map((staffMember) => (
          <tr
            ...
            className={`align-middle ${
              staffMember.status === "Discontinued" ? "opacity-50" : ""
            }`}
          >
            <td className="p-3">
              <div className="d-flex align-items-center">
                <img
                  src={
                    ...
                    `https://ui-avatars.com/api/?name=${staffMember.name.replace(
                      " ",
                      "+"
                    )}&background=random`
                  }
                ...
                />
                <button
                  onClick={() => onViewProfile(staffMember.id)}
