...
const BatchesView: React.FC<{ data: ReturnType<typeof useMockData> }> = ({ data }) => {
  ...
  const initialFormState: Omit<Batch, 'id'> = {
    name: '',
    courseId: '',
    staffId: '',
    weekdays: [],
  ...
  const getStudentCountForBatch = (batchId: string) => students.filter((s) => s.batchIds?.includes(batchId)).length;

  const trainersForCourse = useMemo(
    () => staff.filter((t) => t.role === 'Trainer' && t.expertise?.includes(formState.courseId)),
    [staff, formState.courseId]
  );
          ...
          <tr key={batch.id}>
            ...
            <td className="p-3">
              <div className="d-flex gap-2">
                <button onClick={() => handleOpenModal(batch)} className="btn btn-sm btn-outline-secondary">
                  <EditIcon />
                </button>
                <button onClick={() => setBatchToDelete(batch)} className="btn btn-sm btn-outline-danger">
                  <DeleteIcon />
                </button>
              </div>
            </td>
      ...
      <Modal show={isModalOpen} title={editingBatch ? 'Edit Batch' : 'Add New Batch'} onClose={handleCloseModal}>
        ...
        >
          ...
          <div style={{ maxHeight: '70vh', overflowY: 'auto', paddingRight: '8px' }}>
            ...
            <div className="row">
              <div className="col-md-4">
                <FormInput label="Time (e.g., 08:00 - 10:00)" name="time" value={formState.time} onChange={handleInputChange} required />
              </div>
              <div className="col-md-4">
                <FormInput label="Start Date" name="startDate" type="date" value={formState.startDate} onChange={handleInputChange} required />
              </div>
              <div className="col-md-4">
          ...
          <div className="d-flex justify-content-end pt-3 mt-3 border-top">
            <button type="button" onClick={handleCloseModal} className="btn btn-secondary me-2">
              Cancel
            </button>
