...
const EditIcon: React.FC<{ className?: string }> = (props) => (
  <svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
    ...
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
      ...
      3.536L6.5 21.036H3v-3.5L15.232 5.232z" />
  </svg>
);
    ...
    <div className="card shadow-sm h-100">
      <div className="card-body d-flex flex-column">
        <div className="d-flex align-items-center mb-3">
          ...
          <span className="badge rounded-pill text-bg-light">{course.level}</span>
        </div>
        <h3 className="card-title h5 mb-1">{course.name}</h3>
        <p className="card-text text-body-secondary mb-0">Duration: {course.duration}</p>
      </div>
      <div className="card-footer d-flex justify-content-between align-items-center">
        <span className="h5 mb-0 text-primary fw-bold">₹{course.totalFee.toLocaleString()}</span>
        <div className="d-flex gap-2">
          <button onClick={onEdit} className="btn btn-sm btn-outline-secondary"><EditIcon /></button>
          <button onClick={onDelete} className="btn btn-sm btn-outline-danger"><DeleteIcon /></button>
        </div>
      </div>
    </div>
  );
...
const CoursesView: React.FC<{ data: ReturnType<typeof useMockData> }> = ({ data }) => {
  ...
  const handleSubmit = () => {
    if (formState.name && formState.duration && formState.totalFee > 0) {
      editingCourse ? updateCourse(formState as Course) : addCourse(formState);
      handleCloseModal();
    } else {
          ...
          <div className="row">
            ...
            <div className="col-md-6">
              <FormInput
                ...
                name="totalFee"
                type="number"
                value={formState.totalFee > 0 ? formState.totalFee : ''}
                onChange={handleInputChange}
                required
              />
            </div>
